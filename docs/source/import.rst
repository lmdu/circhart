...
The genome synteny analysis results can be used to prepare link data. Circhart supports the import of result file generated by BLAST, MUMmer, JCVI and MCScanX.


Import BLAST Alignment File
^^^^^^^^^^^^^^^^^^^^^^^^^^^

Circhart can parse the BLAST alignment file generated with parameter ``-outfmt 6``. The example command for running BLAST:

.. code::

	blastn -query query.fasta -subject subject.fasta -evalue 1e-10 -outfmt 6 -out alignment.txt
